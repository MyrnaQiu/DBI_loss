...
class Transformations:
    ...
    def __init__(self, name):
        ...
        self.scale_size = (160, 160, 48)
        self.patch_size = (128, 128, 48)
    ...
    def initialise(self, opts):
        ...
        if hasattr(t_opts, 'val_scale_size'):       self.val_scale_size = t_opts.val_scale_size
        if hasattr(t_opts, 'val_patch_size'):       self.val_patch_size = t_opts.val_patch_size
    ...
    def pros_3d_transform(self):
        ...
        valid_transform = ts.Compose([ts.PadNumpy(size=self.val_scale_size),
                                      ts.ToTensor(),
                                      ts.ChannelsFirst(),
                                      ts.TypeCast(['float', 'float']),
                                      # ts.NormalizeMedicPercentile(norm_flag=(True, False)),
                                      ...
                                      ts.ChannelsLast(),
                                      ts.AddChannel(axis=0),
                                      ts.SpecialCrop(size=self.val_patch_size, crop_type=0),
                                      ts.TypeCast(['float', 'long'])
                                      ])
