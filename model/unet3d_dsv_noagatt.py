...
#skip connection: cbam+ag

class unet3d_dsv_noagatt(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=2, in_channels=1, attention_dsample=(2, 2, 2), is_pooling=True,
        ...
        super(unet3d_dsv_noagatt, self).__init__()
        ...
        self.conv1 = UnetConv3(filters[0], filters[1], self.is_dethwise, 2*filters[0],
                               nn.LeakyReLU(inplace=True), self.att1, self.is_res, kernel_size=(3, 3, 3), padding_size=(1, 1, 1),
                               stride=(1, 1, 1))
        self.conv2 = UnetConv3(filters[1], filters[2], self.is_dethwise, 2*filters[1],
                               nn.LeakyReLU(inplace=True), self.att2, self.is_res, kernel_size=(3, 3, 3), padding_size=(1, 1, 1),
                               stride=(1, 1, 1))
        self.conv3 = UnetConv3(filters[2], filters[3], self.is_dethwise, 2*filters[2],
                               nn.LeakyReLU(inplace=True), self.att3, self.is_res, kernel_size=(3, 3, 3), padding_size=(1, 1, 1),
                               stride=(1, 1, 1))
        self.conv4 = UnetConv3(filters[3], filters[4], self.is_dethwise, 2*filters[3],
                               nn.LeakyReLU(inplace=True), self.att4, self.is_res, kernel_size=(3, 3, 3), padding_size=(1, 1, 1),
                               stride=(1, 1, 1))
