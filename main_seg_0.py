...
class main_seg():
    def __init__(self, json_filename):
        ...
        # nolinear=None,CBAM,SE,CSHWDM,SHWDCM
        ...
        if self.train_opts.is_train == True:
            ...
            if self.json_opts.model.criterion == 'dice_focal':
                ...
                self.criterion = Softdice_dbi_loss(self.model_opts.output_nc, self.e_weight, gamma=self.gamma,
                                                        size_average=True)
    ...
    def train(self):
        ...
        save_path = os.path.join(os.getcwd(), 'save/seg', self.model_type)
    ...
    def test(self, model, test_loader, model_file, epoch, visual_att, visual_edge, is_attsave, is_save):
        ...
        for iter, (images, labels) in tqdm(enumerate(test_loader, 1), total=len(test_loader)):
            ...
            if is_save == True:
                ...
                mu2 = 3
                theta_r = 2
                pred_seg = apply_crf(images[0].cpu().data.numpy().squeeze(0), logits[0].cpu().data.numpy(), theta_a, theta_b, theta_r, mu1, mu2)
                ...
                if name == 'prostate_16.nii.gz' or name == 'Prostate3T-01-0017.nrrd':
                    spacing = [0.75, 0.75, 1.5]
                else:
                ...
                # post operation
                '''
                predict_pri2=copy.deepcopy(predict_pri)
                predict_pri2[np.where(predict_pri2==2)]=1


                img_post=post_operation(predict_pri2)             

                predic = img_post * predict_pri
                '''
                ...
                save_path = './prediction/{}/'.format(self.model_type)
                if not os.path.exists(save_path):
                    os.mkdir(save_path)
                ...
                # attention visual
                '''
                if is_attsave == True and (iter ==1):
                    layer_name1 = 'attentionblock1'
                    #layer_name2 = 'attentionblock3'
                    #layer_name3 = 'attentionblock4'
                    pltSingleHeatmap(model,layer_name1,images)
                    #pltHeatmap(model,layer_name2,images)
                    #pltHeatmap(model,layer_name3,images)
                '''
        ...
        if is_save == True:
            ...
            save_path = os.path.join(os.getcwd(), 'save/seg', self.model_type, '{}.csv'.format(self.model_type))
...
if __name__ == '__main__':
    ...
    # configs = ['config_noag_cbam_dice.json','config_noag_none_ce_meyer.json','config_noag_none_dice.json','config_noag_none_dice_ce.json','config_noag_none_dice_focal.json','config_noag_none_dice_wegce.json','config_noag_none_dice_wegfocal.json','config_noag_none_focal.json','config_noag_none_wegce.json','config_noag_none_wegfocal.json']
    configs = ['config_ag_dice_ce_meyertrain.json','config_ag_dice_meyertrain.json']
